...
Secondly, boot a VM under the doctor project, and check the VM status for the
VM is launched to complete. Then get the compute host info which the VM is
launched to verify connectivity to the target compute host according to
different installers. Get the consumer ip according to the route to compute ip,
create an alarm event in Ceilometer using the consumer ip.
