...
availability service running on top of that host, needs to make an immeditate
switchover from the faulty host to an active standy host. Now, if the compute
...
user can get additional state information about a host in case other existing
state information about VM state is not enough.
