...
make_performance_profile() {
    if [[ "$PROFILER_TYPE" == "poc" ]]; then
      ...
      # TODO(yujunz) compare triggered to linkdown to detect time sync deviation
      export DOCTOR_PROFILER_T00=${triggered}
      export DOCTOR_PROFILER_T01=$(python -c \
          "print(int(($detected-$triggered)*1000))")
      export DOCTOR_PROFILER_T03=$(python -c \
          "print(int(($vmdown-$triggered)*1000))")
      export DOCTOR_PROFILER_T04=$(python -c \
          "print(int(($hostdown-$triggered)*1000))")
      export DOCTOR_PROFILER_T09=$(python -c
          "print(int(($notified-$triggered)*1000))")
