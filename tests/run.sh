...
congress_setup_rules() {
    congress_add_rule nic_down classification \
        'nic_down(host) :-
            doctor:events(hostname=host, type="host.nic1.down")'

    congress_add_rule host_force_down classification \
        'execute[nova:services.force_down(host, "nova-compute")] :-
            nic_down(host)'

    congress_add_rule error_vms_states classification \
        'execute[nova:servers.reset_state(vm)] :-
            nova:servers(id=vm, host_name=host),
            nic_down(host)'
            #nova:services(host=x, binary="nova-compute", state="down")'
}
...
start_inspector() {
    if [[ "$INSPECTOR_TYPE" == "sample" ]] ; then
        ...
        # TODO(cgoncalves): nova DS api_version=2.11
        congress_setup_rules
    fi
}
...
cleanup() {
    ...
    congress_del_rule error_vms_states classification
    congress_del_rule host_force_down classification
    congress_del_rule nic_down classification
}
