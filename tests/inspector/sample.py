...
class SampleInspector(BaseInspector):
    ...
    def _init_novaclients(self):
        session = get_session(project=self.conf.project_name)
    ...
    def start(self):
        ...
        self.app = inspector_app(self.conf.inspector.port, self, self.log)
        self.app.start()
    ...
    def stop(self):
        ...
        url = '%s%s' % (self.conf.inspector.url, 'shutdown') \
            if self.inspector_url.endswith('/') else \
            '%s%s' % (self.conf.inspector.url, '/shutdown')
