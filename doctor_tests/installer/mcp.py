...
class McpInstaller(BaseInstaller):
    ...
    def get_node_ips(self):
        ...
        command = "sudo salt --out yaml 'ctl01*' " \
                  "pillar.get _param:openstack_control_address |" \
                  "awk '{print $2}'"
        controller0 = self._run_cmd_remote(self.client, command)
        addr_space = controller0[0].rsplit(".", 1)[0]
        command = 'sudo salt \* network.ip_addrs'
        node_details = self._run_cmd_remote(self.client, command)

        prev_line = None
        for line in node_details:
            if ":" in line:
                prev_line = line.split(":")[0]
                continue
            elif addr_space not in line:
                continue
            if prev_line is not None:
                if "ctl" in prev_line:
                    self.controllers.append(line.split()[1])
                elif "cmp" in prev_line:
                    self.computes.append(line.split()[1])
                prev_line = None
                continue
