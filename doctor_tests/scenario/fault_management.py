...
class FaultManagement(object):
    ...
    def check_notification_time(self):
        notification_time = \
            float(self.consumer.notified_time) -\
            float(self.monitor.detected_time)
    ...
    def run_profiler(self):
        ...
        net_down_log_file = self.fault_management.get_disable_network_log()
        reg = '(?<=doctor set link down at )\d+.\d+'
        linkdown = float(match_rep_in_file(reg, net_down_log_file).group(0))

        vmdown = float(self.inspector.vm_down_time)
        hostdown = float(self.inspector.host_down_time)
        detected = float(self.monitor.detected_time)
        notified = float(self.consumer.notified_time)
