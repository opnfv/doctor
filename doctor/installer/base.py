...
_installer_name_class_mapping = {
    'apex': 'doctor.installer.apex.ApexInstaller',
    'fuel': 'doctor.installer.fuel.FuelInstaller',
    'local': 'doctor.installer.local.LocalInstaller'
}

def get_installer(conf):
    if conf.installer.type not in SUPPORTED_INSTALLER_TYPES:
        raise Exception("Installer type '%s' not supported", conf.installer.type)
